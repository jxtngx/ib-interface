...
class TickerTable(qt.QTableWidget):
    ...
    headers = ["symbol", "bidSize", "bid", "ask", "askSize", "last", "lastSize", "close"]
    ...
    def addTicker(self, ticker):
        ...
        item.setText(ticker.contract.symbol + (ticker.contract.currency if ticker.contract.secType == "CASH" else ""))
...
class Window(qt.QWidget):
    ...
    def add(self, text=""):
        ...
        if text:
            contract = eval(text)
            if contract and self.ib.qualifyContracts(contract) and contract not in self.table:
    ...
    def onConnectButtonClicked(self, _):
        if self.ib.isConnected():
            ...
            for symbol in ("EURUSD", "USDJPY", "EURGBP", "USDCAD", "EURCHF", "AUDUSD", "NZDUSD"):
