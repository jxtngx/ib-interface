...
@pytest.mark.skip(reason="needs to connect to TWS")
async def test_request_error_raised():
    contract = ibi.Forex("EURUSD")
    order = ibi.MarketOrder("BUY", 100)
    orderState = await ibi.whatIfOrderAsync(contract, order)
    assert orderState.commission > 0

    ibi.RaiseRequestErrors = True
    badContract = ibi.Stock("XXX")
    with pytest.raises(ibi.RequestError) as exc_info:
        await ibi.whatIfOrderAsync(badContract, order)
